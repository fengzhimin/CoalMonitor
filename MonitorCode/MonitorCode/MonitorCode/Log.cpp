...
#define ERROR_FILE  L"./error.log";
#define OPERATE_FILE  L"./operate.log";
...
{
	CString _temp;
	GetPrivateProfileStringW(_T("Load Log Info"), _T("Config"), _T("FALSE"), _temp.GetBuffer(MAX_PATH),MAX_PATH,_T("./Config.ini"));
	_temp.ReleaseBuffer();
	if(_temp == L"TRUE")
		m_loadLog = true;
	else
...
void Log::WriteErrorLog(CString strErrorLogInfo)
{
	if(m_loadLog)
	{
		...
		try
		{
			...
			strData = L"Error: " + tt.Format("[%Y-%B-%d %A, %H:%M:%S] ") + strErrorLogInfo;
			strData += "\n";
...
void Log::WriteOperateLog(CString strOperateLogInfo)
{
	if(m_loadLog)
	{
		...
		try
		{
			...
			CString strData;
			CTime tt = CTime::GetCurrentTime();
			strData = L"Operate: " + tt.Format("[%Y-%B-%d %A, %H:%M:%S] ") + strOperateLogInfo;
			strData += "\n";
			char* old_locale = _strdup( setlocale(LC_CTYPE,NULL) );
			setlocale( LC_CTYPE, "chs" );//新的区域设定
